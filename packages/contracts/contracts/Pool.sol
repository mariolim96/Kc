...
import './interfaces/ICarbonOffsetToken.sol';
    ...
    PoolStorage
{
    ...
    event TCO2Bridged(uint32 indexed destinationDomain, address indexed tco2, uint256 amount);
    ...
    function onlyWithRole(bytes32 role) internal view virtual {
        require(hasRole(role, msg.sender) || owner() == msg.sender, Errors.CP_UNAUTHORIZED);
    }
    ...
    function setFeeRedeemBurnPercentage(uint256 _feeRedeemBurnPercentageInBase) external virtual {
        onlyPoolOwner();
        require(_feeRedeemBurnPercentageInBase < feeRedeemDivider, Errors.CP_INVALID_FEE);
    ...
    function setFeeRedeemBurnAddress(address _feeRedeemBurnAddress) external virtual {
        onlyPoolOwner();
    ...
    function calculateRedeemFees(
        ...
        for (uint256 i; i < tco2Length; ++i) {
            uint256 feeAmount = (amounts[i] * _feeRedeemPercentageInBase) / feeRedeemDivider;
            totalFee += feeAmount;
        }
        return totalFee;
    }
    ...
    function redeemAndBurn(address tco2, uint256 amount) external {
        ...
        ICarbonOffsets(tco2).burnFrom(msg.sender, amount);
    }
    ...
    function redeemAndRetireMany(
        address[] memory tco2s,
        uint256[] memory amounts
    ) external virtual returns (uint256[] memory retirementIds, uint256[] memory redeemedAmounts) {
        (retirementIds, redeemedAmounts) = redeemManyInternal(tco2s, amounts, true);
    }
    ...
    function redeemManyInternal(
        ...
        uint256[] memory amounts,
        bool toRetire
    ) internal returns (uint256[] memory retirementIds, uint256[] memory redeemedAmounts) {
        onlyUnpaused();
        uint256 tco2Length = tco2s.length;
        require(tco2Length == amounts.length, Errors.CP_LENGTH_MISMATCH);
        ...
        for (uint256 i; i < tco2Length; ) {
            ...
            if (!isExempted) {
                feeAmount = (amounts[i] * _feeRedeemPercentageInBase) / feeRedeemDivider;
                totalFee += feeAmount;
            } else {
                feeAmount = 0;
            }
            ...
            // on behalf of them
            if (toRetire) {
                retirementIds[i] = ICarbonOffsets(tco2s[i]).retireFrom(msg.sender, amountToRedeem);
            }
        ...
        if (totalFee != 0) {
            uint256 burnAmount = (totalFee * feeRedeemBurnPercentageInBase) / feeRedeemDivider;
            totalFee -= burnAmount;
    ...
    function redeemAuto(uint256 amount) external virtual returns (address[] memory tco2s, uint256[] memory amounts) {
    ...
    function tokenBalances(address tco2) public view returns (uint256) {
        return IERC20(tco2).balanceOf(address(this));
    }
