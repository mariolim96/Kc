    ...
    PoolFilterStorage
{
    ...
    function checkEligible(address erc20Addr) external view virtual returns (bool) {
        bool isToucanContract = IRegistry(contractRegistry).isValidERC20(erc20Addr);
    ...
    function checkAttributeMatching(address erc20Addr) public view virtual returns (bool) {
        ProjectData memory projectData;
        VintageData memory vintageData;
        (projectData, vintageData) = ICarbonOffsets(erc20Addr).getAttributes();

        /// @dev checks if any one of the attributes are blacklisted.
        /// If mappings are set to "whitelist"-mode, require the opposite
        require(vintageData.startTime >= minimumVintageStartTime, Errors.CP_START_TIME_TOO_OLD);
        require(regions[projectData.region] == regionsIsAcceptedMapping, Errors.CP_REGION_NOT_ACCEPTED);
        require(standards[projectData.standard] == standardsIsAcceptedMapping, Errors.CP_STANDARD_NOT_ACCEPTED);
        require(
            methodologies[projectData.methodology] == methodologiesIsAcceptedMapping,
    ...
    function switchMapping(string memory _mappingName, bool accepted) external virtual {
        onlyPoolOwner();
        if (_mappingName.equals('regions')) {
            accepted ? regionsIsAcceptedMapping = true : regionsIsAcceptedMapping = false;
        } else if (_mappingName.equals('standards')) {
            accepted ? standardsIsAcceptedMapping = true : standardsIsAcceptedMapping = false;
        } else if (_mappingName.equals('methodologies')) {
            accepted ? methodologiesIsAcceptedMapping = true : methodologiesIsAcceptedMapping = false;
        }
    ...
    function setMinimumVintageStartTime(uint64 _minimumVintageStartTime) external virtual {
        onlyPoolOwner();
