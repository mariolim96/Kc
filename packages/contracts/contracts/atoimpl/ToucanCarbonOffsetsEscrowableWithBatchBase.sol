...
// abstract contract ToucanCarbonOffsetsEscrowableWithBatchBase is IERC721Receiver, ToucanCarbonOffsetsWithBatchBase {
//     // ----------------------------------------
//     //       Events
//     // ----------------------------------------

//     event DetokenizationRequested(address indexed user, uint256 amount, uint256 indexed requestId, uint256[] batchIds);
//     event DetokenizationReverted(uint256 indexed requestId);
//     event DetokenizationFinalized(uint256 indexed requestId);

//     event RetirementRequested(address indexed user, uint256 indexed requestId, CreateRetirementRequestParams params);
...
//     function finalizeDetokenization(uint256 requestId) external whenNotPaused onlyWithRole(DETOKENIZER_ROLE) {
//         address escrow = IToucanContractRegistry(contractRegistry).toucanCarbonOffsetsEscrowAddress();

//         // Fetch batch NFT IDs from escrow request
//         DetokenizationRequest memory request = IToucanCarbonOffsetsEscrow(escrow).detokenizationRequests(requestId);

//         _updateBatchStatuses(request.batchTokenIds, BatchStatus.DetokenizationFinalized);
//         // Finalize escrow request
//         IToucanCarbonOffsetsEscrow(escrow).finalizeDetokenizationRequest(requestId);
...
//     /// @param requestId The request id in the escrow contract that
//     /// tracks the detokenization request
//     function revertDetokenization(uint256 requestId) external whenNotPaused onlyWithRole(DETOKENIZER_ROLE) {
//         address escrow = IToucanContractRegistry(contractRegistry).toucanCarbonOffsetsEscrowAddress();
...
//         IToucanCarbonOffsetsEscrow(escrow).revertDetokenizationRequest(requestId);
...
//     function finalizeRetirement(uint256 requestId) external whenNotPaused onlyWithRole(RETIREMENT_ROLE) {
//         address escrow = IToucanContractRegistry(contractRegistry).toucanCarbonOffsetsEscrowAddress();

//         // Fetch batch NFT IDs from escrow request
//         RetirementRequest memory request = IToucanCarbonOffsetsEscrow(escrow).retirementRequests(requestId);

//         _updateBatchStatuses(request.batchTokenIds, BatchStatus.RetirementFinalized);
//         // Finalize escrow request
...
//     /// @param requestId The request id in the escrow contract that
//     /// tracks the retirement request
//     function revertRetirement(uint256 requestId) external whenNotPaused onlyWithRole(RETIREMENT_ROLE) {
//         address escrow = IToucanContractRegistry(contractRegistry).toucanCarbonOffsetsEscrowAddress();
...
//         address escrow = IToucanContractRegistry(contractRegistry).toucanCarbonOffsetsEscrowAddress();
//         uint256 totalAmount = _updateBatchStatuses(tokenIds, BatchStatus.DetokenizationRequested);
...
//         uint256 requestId = IToucanCarbonOffsetsEscrow(escrow).createDetokenizationRequest(
//             _msgSender(),
//             amount,
//             tokenIds
//         );
...
//         address escrowAddress = IToucanContractRegistry(contractRegistry).toucanCarbonOffsetsEscrowAddress();
//         uint256 totalAmount = _updateBatchStatuses(params.tokenIds, BatchStatus.RetirementRequested);
...
//         // Create escrow contract request
//         require(approve(escrowAddress, params.amount), Errors.TCO2_APPROVAL_AMT_FAILED);
//         requestId = IToucanCarbonOffsetsEscrow(escrowAddress).createRetirementRequest(_msgSender(), params);
...
//     function _updateBatchStatuses(uint256[] memory tokenIds, BatchStatus status) internal returns (uint256) {
//         address batchNFT = IToucanContractRegistry(contractRegistry).carbonOffsetBatchesAddress();
//         uint256 totalAmount;
...
//             (, uint256 batchAmount, ) = _getNormalizedDataFromBatch(batchNFT, tokenId);
//             totalAmount += batchAmount;
//             // Transition batch status to updated status
//             ICarbonOffsetBatches(batchNFT).setStatusForDetokenizationOrRetirement(tokenId, status);
//             unchecked {
//                 ++i;
//             }
//         }
