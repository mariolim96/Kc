...
// abstract contract ToucanCarbonOffsetsWithBatchBase is IERC721Receiver, ToucanCarbonOffsetsBase {
//     // ----------------------------------------
//     //       Admin functions
//     // ----------------------------------------
...
//         (, uint256 batchAmount, BatchStatus status) = _getNormalizedDataFromBatch(batchNFT, tokenId);
//         require(status == BatchStatus.Confirmed, Errors.TCO2_BATCH_NOT_CONFIRMED);
...
//         require(checkWhiteListed(msg.sender), Errors.TCO2_BATCH_NOT_WHITELISTED);

//         (uint256 gotVintageTokenId, uint256 quantity, BatchStatus status) = _getNormalizedDataFromBatch(
//             msg.sender,
//             tokenId
//         );
//         require(gotVintageTokenId == _projectVintageTokenId, Errors.TCO2_NON_MATCHING_NFT);
//         require(status == BatchStatus.Confirmed, Errors.TCO2_BATCH_NOT_CONFIRMED);
...
//             IToucanContractRegistry(contractRegistry).toucanCarbonOffsetsFactoryAddress(standardRegistry())
//         );
...
//             (uint256 feeAmount, uint256 feeBurnAmount) = tco2Factory.getBridgeFeeAndBurnAmount(quantity);
...
//     function _getNormalizedDataFromBatch(
//         address cob,
//         uint256 tokenId
//     ) internal view returns (uint256, uint256, BatchStatus) {
//         (uint256 vintageTokenId, uint256 quantity, BatchStatus status) = ICarbonOffsetBatches(cob).getBatchNFTData(
//             tokenId
//         );
...
//         if (collection == IToucanContractRegistry(contractRegistry).carbonOffsetBatchesAddress()) {
//             return true;
//         } else {
//             return false;
//         }
